...
int main (int argc, char * argv[])
{
  ...
  for (i=0;i<256;i++) {
    storage[i] = malloc(256*sizeof(unsigned int));
    memset(storage[i],0,256);
  }
  image = malloc(256*sizeof(unsigned char *));
  for (i=0;i<256;i++){image[i]=malloc(256*sizeof(unsigned char));}

  puts("calculation..");
  
  for (counter=0;counter<MAX_ITERATIONS;counter++)
  {
    /* get 256 first xoring bytes from random key */
    data = rc4_bytes(init_buffer(random_buffer(),256),256);
    /* add to storage */
    for (i=0;i<256;i++){storage[data[i]][i]++;}
  }
  ...
  puts("rendering..");
  /* make image */
  for (i=0;i<256;i++){for (j=0;j<256;j++){image[i][j] = (unsigned char)(0xff*(storage[i][j]/(float)counter));}}


  
  write_ppm(image,(unsigned char *)"output.ppm");
  return EXIT_SUCCESS;
}
